...
const Work = () => {
  const projects = [
    {
      ...
      imagePlaceholder: "Naija Trade Hub screenshot"
    },
    {
      tags: ["Fintech", "SaaS"],
      title: "TipMe",
  ...
  return (
    <section id="work" className="py-24 lg:py-32 px-6 lg:px-12 max-w-7xl mx-auto border-t-4 border-ink relative">
      ...
      >
        <h2 className="font-sans font-bold text-4xl lg:text-5xl text-ink tracking-tight">Things I've built</h2>
      </motion.div>
            ...
            <div className="p-6 flex flex-col flex-grow">
              <div className="flex flex-wrap gap-2 mb-4">
                {project.tags.map((tag, i) => (
                  <span key={i} className="font-sans font-bold text-xs uppercase tracking-wide bg-card text-ink border-2 border-ink px-2 py-0.5 shadow-brutal-sm">
                    {tag}
                  </span>
                ))}
              </div>
              
              <h3 className="font-sans font-bold text-2xl text-ink mb-3 tracking-tight">{project.title}</h3>
              <p className="font-sans font-medium text-sm text-ink leading-relaxed mb-6 flex-grow">{project.desc}</p>
              
              <div className="mt-auto">
                <span className={`inline-flex items-center gap-1 font-sans text-xs font-bold uppercase tracking-wider px-4 py-2 hover:translate-y-[1px] hover:translate-x-[1px] hover:shadow-none transition-all cursor-pointer ${project.statusClass}`}>
                  {project.status}
                </span>
              </div>
            </div>
